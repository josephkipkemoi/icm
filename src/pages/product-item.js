...
const ProductItemComponent = ({ setUpdateCart }) => {
    ...
    const validateProductCart = () => {
        if (size === null) {
            setValidationError(["You need to pick at least one package size before proceeding."])
            return false
        }
        return true
    }

    const handleShare = (product_name) => {
        const isValidated = validateProductCart()
        if(isValidated) {
            const packageSize = size + "g"
            const productName = product_name
            const message = `Greetings ICM, I would like to inquire about ${productName} ${packageSize}`
        ...
        <div className="container mt-5 mb-5 p-3">
            <div className="row">
                ...
                <div className="col-sm m-2 product-container">
                    ...
                    {validationError.length > 0 && validationError.map((v,i) => <small key={v+i} className="alert alert-danger border-0 d-block p-2 text-center">{v}</small>)}
                    {product_package_size.map((val,i) => <button className="btn btn-dark m-1" key={i} onClick={() => handlePickedSize(val)}>{val}g</button>)}
                    <div className="d-sm-flex align-items-center">
                        <a href={`https://wa.me/send?phone=254795216873&text=${productMessage}`} target="_blank" rel="noreferrer" className="btn btn-warning m-1" onClick={() => handleShare(product_name)}>
